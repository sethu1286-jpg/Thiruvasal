...
.bottom-nav { position:fixed; bottom:0; left:50%; transform:translateX(-50%); width:100%; max-width:430px; background:rgba(10,22,40,0.98); border-top:1px solid rgba(255,140,0,0.15); display:flex; justify-content:space-around; padding:10px 6px 14px; z-index:500; backdrop-filter:blur(12px); }
.bottom-btn { flex:1; background:transparent; border:none; color:rgba(155,181,224,0.55); font-family:inherit; cursor:pointer; display:flex; flex-direction:column; align-items:center; gap:4px; font-size:11px; font-weight:700; }
.bottom-btn.active { color:#FFA833; }
.bottom-btn .icon { font-size:18px; }
.loader-wrap { min-height:100vh; display:flex; align-items:center; justify-content:center; background:#0F1B3D; }
`;
...
function TopBar({ title, sub, onBack }) {
  return (
    ...
    >
      ...
      <div>
        <div style={{ color: "#F0F6FF", fontWeight: 800, fontSize: 16 }}>{title}</div>
        {sub ? (
          <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11, marginTop: 2 }}>{sub}</div>
        ) : null}
      </div>
    </div>
  );
...
function Sun({ size = 90 }) {
  ...
  return (
    <svg
      ...
      style={{
        overflow: "visible",
        filter: `drop-shadow(0 0 ${size * 0.12}px rgba(255,140,0,0.8))`,
        margin: "0 auto",
      }}
    >
      <defs>
        <radialGradient id="sg">
          <stop offset="0%" stopColor="#FFA833" stopOpacity="0.5" />
      ...
      <g style={{ transformOrigin: `${c}px ${c}px`, animation: "rayRot 18s linear infinite" }}>
        {[...Array(12)].map((_, i) => {
          ...
          return (
            <line
              key={i}
              x1={c + Math.cos(a) * r * 1.2}
              y1={c + Math.sin(a) * r * 1.2}
              x2={c + Math.cos(a) * r * 2.1}
              y2={c + Math.sin(a) * r * 2.1}
              stroke="#FFD080"
...
function Login() {
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background: "radial-gradient(ellipse at 50% 25%,#162347,#0F1B3D 50%,#080F20)",
        display: "flex",
    ...
    >
      ...
      <div style={{ padding: "56px 24px 20px", textAlign: "center", animation: "sunUp 0.8s ease forwards" }}>
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 10, letterSpacing: 4, color: "rgba(255,168,51,0.55)", marginBottom: 6 }}>
            AYYAAKILAM
          </div>
          <div className="gold" style={{ fontSize: 38, fontWeight: 900, lineHeight: 1.1 }}>
            THIRUVASAL
          </div>
          <div style={{ color: "rgba(155,181,224,0.55)", fontSize: 13, marginTop: 8 }}>
      ...
      <div
        style={{
          flex: 1,
          background: "linear-gradient(180deg,rgba(22,35,71,0),rgba(12,20,48,0.98) 12%)",
      ...
      >
          ...
          {[["login", "Login"], ["register", "Register"]].map(([m, l]) => (
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                fontSize: 13,
                background: mode === m ? "linear-gradient(135deg,#FF8C00,#FFA833)" : "transparent",
                color: mode === m ? "#0A1628" : "rgba(155,181,224,0.6)",
                transition: "all 0.2s",
              }}
            >
              {l}
        ...
        {mode === "register" && (
          <>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#5A7AB5", display: "block", marginBottom: 5 }}>
              Full Name
            </label>
            <input className="inp" placeholder="Your name" value={form.name} onChange={s("name")} style={{ marginBottom: 12 }} />
          </>
        )}
        ...
        <input
          ...
          value={form.password}
          onChange={s("password")}
          style={{ marginBottom: mode === "register" ? 12 : 20 }}
        />

        {mode === "register" && (
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#5A7AB5", display: "block", marginBottom: 8 }}>
              Role
            </label>
              ...
              {[["donor", "Donor"], ["admin", "Admin"]].map(([v, l]) => (
                <button
                  ...
                  style={{
                    ...
                    padding: "10px",
                    borderRadius: 10,
                    border: `1.5px solid ${form.role === v ? "#FF8C00" : "rgba(255,140,0,0.2)"}`,
                    background: form.role === v ? "rgba(255,140,0,0.12)" : "transparent",
                    color: form.role === v ? "#FFA833" : "rgba(155,181,224,0.5)",
                    fontWeight: 700,
                    fontSize: 12,
                    cursor: "pointer",
                    fontFamily: "inherit",
        ...
        >
          {loading ? (
            <>
              <div className="spin" style={{ borderTopColor: "#0A1628" }} />
        ...
        >
          <span style={{ fontSize: 11, color: "rgba(155,181,224,0.45)" }}>
...
function Home({ setTab }) {
  ...
  useEffect(() => {
    try {
      ...
      return onSnapshot(q, (s) => setDonors(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
    } catch (e) {
      console.error(e);
    }
  }, []);
  ...
  return (
    <div className="scroll" style={{ paddingBottom: 90 }}>
      <div
        style={{
          background: "radial-gradient(ellipse at 50% 0%,#1E2F5C,#0F1B3D 60%)",
      ...
      >
        ...
        <Sun size={88} />
        <div className="gold" style={{ fontSize: 26, fontWeight: 900, marginTop: 12 }}>
          THIRUVASAL
        </div>
        <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 12, marginTop: 4 }}>
          Welcome, {profile?.name || "User"}
        </div>

        <div style={{ display: "flex", gap: 9, marginTop: 20 }}>
          {[
            { i: "👥", l: "Donors", v: donors.length },
            { i: "⏳", l: "Pending", v: pending.length, c: "#FF9800" },
            { i: "💰", l: "Collected", v: total >= 1000 ? (total / 1000).toFixed(0) + "K" : total },
          ].map((s) => (
            <div
              key={s.l}
              style={{
                flex: 1,
                background: "rgba(22,35,71,0.7)",
      ...
      <div style={{ padding: "18px 16px 0" }}>
        <div
          className="card fade"
          style={{
            background: "linear-gradient(135deg,rgba(255,140,0,0.13),rgba(22,35,71,0.9))",
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div className="gold" style={{ fontWeight: 800, fontSize: 16 }}>
                {profile?.name || "User"}
              </div>
              <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11 }}>
                {profile?.phone || ""} | {profile?.role === "admin" ? "Admin" : "Donor"}
              </div>
            </div>
          </div>
        </div>

        <div style={{ fontSize: 10, letterSpacing: 2, color: "#5A7AB5", fontWeight: 700, marginBottom: 10, marginTop: 6 }}>
          MAIN FEATURES
        </div>
        ...
        {pending.length > 0 && (
          ...
          >
            ...
            <div>
              <div style={{ fontWeight: 700, color: "#FF9800", fontSize: 13 }}>{pending.length} Pending Donations</div>
              <div style={{ fontSize: 11, color: "rgba(255,152,0,0.55)" }}>Confirmation required</div>
            </div>
          </div>
        )}
        ...
        >
          ...
          <div style={{ color: "rgba(255,168,51,0.7)", fontSize: 12, fontStyle: "italic" }}>
            He who gives shall receive a thousandfold
          </div>
        </div>
      </div>
    </div>
...
function Charity() {
  ...
  useEffect(() => {
    try {
      const q = query(collection(db, "donors"), orderBy("createdAt", "desc"));
      return onSnapshot(q, (snap) => setDonors(snap.docs.map((d) => ({ id: d.id, ...d.data() }))));
    } catch (e) {
      console.error(e);
    }
  }, []);
  ...
  async function addD() {
    ...
    try {
      await addDoc(collection(db, "donors"), {
        ...
        status: "pending",
        addedBy: profile?.uid || "",
        nextReminderDate: form.donationDate,
        createdAt: serverTimestamp(),
    ...
    setLoading(false);
    setShowAdd(false);
    setForm({ name: "", phone: "", amount: "", purpose: "", donationDate: "" });
  }

  async function markStatus(id, status) {
    try {
      await updateDoc(doc(db, "donors", id), { status, updatedAt: serverTimestamp() });
    } catch (e) {
      console.error(e);
    }
    setSel(null);
      ...
      <div style={{ background: "#0F1B3D", minHeight: "100vh" }}>
        ...
        <div style={{ padding: "18px 16px 80px" }} className="scroll">
          <div
            ...
            style={{
              textAlign: "center",
              background: "linear-gradient(135deg,rgba(255,140,0,0.12),rgba(22,35,71,0.9))",
          ...
          >
            ...
            <div className="gold" style={{ fontSize: 20, fontWeight: 800 }}>
              {d.name}
            </div>
            ...
            {[
              { l: "Amount", v: `Rs ${Number(d.amount || 0).toLocaleString()}` },
              { l: "Purpose", v: d.purpose },
              { l: "Date", v: fmtDate(d.donationDate) },
              { l: "Next Reminder", v: fmtDate(d.nextReminderDate) },
            ].map((r) => (
          ...
          {d.status === "pending" && (
            <div className="card" style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 12, marginBottom: 16 }}>
                Will you donate Rs {Number(d.amount).toLocaleString()} this year?
              </div>
              <div style={{ display: "flex", gap: 10 }}>
                <button className="btn-gold" style={{ flex: 1, padding: "13px", fontSize: 14 }} onClick={() => markStatus(d.id, "paid")}>
                  ✅ Yes
                </button>
                <button
                  className="btn-out"
          ...
          {isAdmin && (
            <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
              <button
                ...
                onClick={() => markStatus(d.id, d.status === "paid" ? "pending" : "paid")}
              >
                {d.status === "paid" ? "Mark Pending" : "Mark Paid"}
              </button>
              <button
                className="btn-out"
                style={{ flex: 1, padding: "11px", fontSize: 13, color: "#EF4444", borderColor: "rgba(239,68,68,0.3)" }}
                onClick={() => delDonor(d.id)}
              >
                Delete
              </button>
            </div>
  ...
  return (
    <div style={{ background: "#0F1B3D", minHeight: "100vh" }}>
      <div
        style={{
          background: "radial-gradient(ellipse at 50% 0%,#1E3060,#0F1B3D 70%)",
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div className="gold" style={{ fontSize: 22, fontWeight: 900 }}>Donations</div>
            <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11, marginTop: 2 }}>Donor Management</div>
          </div>
          {isAdmin && (
            <button className="btn-gold" style={{ padding: "9px 16px", fontSize: 13 }} onClick={() => setShowAdd(true)}>
              + Add
            </button>
          )}
        </div>
          ...
          {[["all", "All"], ["pending", "Pending"], ["paid", "Paid"]].map(([f, l]) => (
            <button
              ...
              style={{
                ...
                color: filter === f ? "#0A1628" : "rgba(155,181,224,0.7)",
                fontWeight: 700,
                fontSize: 11.5,
                cursor: "pointer",
                fontFamily: "inherit",
      ...
      <div style={{ padding: "14px 16px 90px" }} className="scroll">
        {filtered.length === 0 ? (
          <div style={{ textAlign: "center", padding: "52px 20px", color: "#5A7AB5" }}>
            <div style={{ fontSize: 44, marginBottom: 12, opacity: 0.5 }}>🏛️</div>
            <div>No donors found</div>
          </div>
        ) : (
          filtered.map((d) => (
            <div key={d.id} className="card" onClick={() => setSel(d.id)} style={{ cursor: "pointer" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      {showAdd && (
        ...
        >
          ...
          >
            ...
            {[
              { k: "name", l: "Full Name", p: "Donor name" },
              { k: "phone", l: "Phone", p: "9XXXXXXXXX", t: "tel" },
              { k: "amount", l: "Amount (Rs)", p: "5000", t: "number" },
              { k: "purpose", l: "Purpose", p: "Festival name" },
              { k: "donationDate", l: "Donation Date", p: "", t: "date" },
            ].map((f) => (
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              <button className="btn-gold" disabled={loading} style={{ flex: 1, padding: "13px", fontSize: 14 }} onClick={addD}>
                {loading ? "Saving..." : "Save Donor"}
              </button>
              <button className="btn-out" style={{ flex: 1, padding: "13px", fontSize: 14 }} onClick={() => setShowAdd(false)}>
                Cancel
              </button>
            </div>
          </div>
...
function Business() {
  ...
  async function reg() {
    ...
    setLoading(false);
    setShow(false);
    setForm({ name: "", service: "", price: "", phone: "", category: "Flowers", plan: "free" });
  }
  ...
  return (
    <div style={{ background: "#0F1B3D", minHeight: "100vh" }}>
      <div
        style={{
          background: "radial-gradient(ellipse at 50% 0%,#0A2040,#0F1B3D 70%)",
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 22, fontWeight: 900, color: "#4A9FFF" }}>Business</div>
            <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11, marginTop: 2 }}>{list.length} listings</div>
          </div>
          <button className="btn-gold" style={{ padding: "9px 14px", fontSize: 13 }} onClick={() => setShow(true)}>
            + Register
          </button>
        </div>
        ...
        >
          <div>
            <div style={{ color: "#FFA833", fontWeight: 800, fontSize: 13 }}>⭐ Premium</div>
            <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11, marginTop: 2 }}>Top placement, more reach</div>
          </div>
          ...
          list.map((b) => (
            <div key={b.id} className="card">
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <div>
                  <div style={{ color: "#F0F6FF", fontWeight: 700, fontSize: 14 }}>{b.name}</div>
                  <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 12, marginTop: 1 }}>{b.service}</div>
                  <div style={{ color: "rgba(155,181,224,0.4)", fontSize: 11 }}>{b.category}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div className="gold" style={{ fontWeight: 800, fontSize: 13 }}>{b.price || "--"}</div>
      ...
      {show && (
        ...
        >
          ...
          >
            ...
            {[
              { k: "name", l: "Business Name", p: "Shop name" },
              { k: "service", l: "Service/Product", p: "What you offer" },
              { k: "price", l: "Price", p: "Rs 100 onwards" },
              { k: "phone", l: "Phone", p: "9XXXXXXXXX", t: "tel" },
            ].map((f) => (
              <div key={f.k} style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: "#5A7AB5", display: "block", marginBottom: 5 }}>
                ...
                {[["free", "Free", "Basic"], ["paid", "Premium", "Rs 299/mo"]].map(([v, l, sub]) => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      textAlign: "center",
                      border: `1.5px solid ${form.plan === v ? "#FF8C00" : "rgba(255,140,0,0.18)"}`,
                      background: form.plan === v ? "rgba(255,140,0,0.1)" : "transparent",
                    }}
                  >
                    <div style={{ fontWeight: 700, color: form.plan === v ? "#FFA833" : "rgba(155,181,224,0.5)", fontSize: 13 }}>
                      {l}
                    </div>
            ...
            <button className="btn-gold" disabled={loading} style={{ width: "100%", padding: "14px", fontSize: 15 }} onClick={reg}>
...
function Profile() {
  ...
  return (
    <div style={{ background: "#0F1B3D", minHeight: "100vh" }}>
      <div
        style={{
          background: "radial-gradient(ellipse at 50% 0%,#1A2F5C,#0F1B3D 70%)",
      ...
      >
        ...
        <div className="gold" style={{ fontSize: 22, fontWeight: 800 }}>
          {profile?.name || "User"}
        </div>
        <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 13, marginTop: 4 }}>
          {profile?.role === "admin" ? "Admin" : "Donor"}
        </div>
      </div>
      ...
      <div style={{ padding: "18px 16px 90px" }}>
        <div className="card">
          ...
          {[
            ...
            { l: "Role", v: profile?.role === "admin" ? "Admin" : "Donor" },
          ].map((r, i) => (
        ...
        <button
          className="btn-out"
          style={{ width: "100%", padding: "14px", fontSize: 14, color: "#EF4444", borderColor: "rgba(239,68,68,0.35)" }}
          onClick={logout}
        >
          Logout
        </button>
...
function Admin() {
  ...
  useEffect(() => {
    try {
      const q = query(collection(db, "donors"), orderBy("createdAt", "desc"));
      const u = onSnapshot(q, (s) => setDonors(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
      return u;
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => {
    try {
      ...
      const u = onSnapshot(q, (s) => setBusinesses(s.docs.map((d) => ({ id: d.id, ...d.data() }))));
      return u;
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div style={{ background: "#0A1020", minHeight: "100vh" }}>
      <div
        style={{
          background: "radial-gradient(ellipse at 50% 0%,#1A0A3C,#0A1020 70%)",
          padding: "20px 20px 22px",
          borderBottom: "1px solid rgba(255,140,0,0.15)",
        }}
      >
          ...
          {[["overview", "Overview"], ["donors", "Donors"], ["business", "Business"]].map(([id, l]) => (
            <button
              ...
              style={{
                ...
                borderRadius: 20,
                border: "none",
                background: tab === id ? "linear-gradient(135deg,#FF8C00,#FFA833)" : "rgba(255,140,0,0.08)",
                color: tab === id ? "#0A1628" : "rgba(155,181,224,0.5)",
                fontWeight: 700,
                fontSize: 11.5,
                cursor: "pointer",
                fontFamily: "inherit",
        ...
        {tab === "overview" && (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              {[
                { i: "✅", l: "Paid", v: paid.length, c: "#22C55E" },
                { i: "⏳", l: "Pending", v: pending.length, c: "#FF9800" },
                { i: "Rs", l: "Collected", v: total.toLocaleString(), c: "#FF8C00" },
                { i: "🛒", l: "Businesses", v: businesses.length, c: "#4A9FFF" },
              ].map((s) => (
                <div
                  key={s.l}
                  style={{
            ...
            <div className="card">
              <div style={{ color: "#5A7AB5", fontSize: 10, letterSpacing: 2, fontWeight: 700, marginBottom: 12 }}>
                AUTO REMINDERS
              </div>
              {[
                "1 month before — SMS + Notification",
                "1 week before — SMS + Notification",
                "On the same day — SMS + Notification",
              ].map((r) => (
            ...
            donors.map((d) => (
              <div key={d.id} className="card">
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ color: "#F0F6FF", fontWeight: 700, fontSize: 14 }}>{d.name}</div>
                    <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 11 }}>📞 {d.phone}</div>
                    <div style={{ color: "rgba(155,181,224,0.4)", fontSize: 11 }}>
                      {d.purpose} | {fmtDate(d.donationDate)}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div className="gold" style={{ fontWeight: 900 }}>Rs {Number(d.amount || 0).toLocaleString()}</div>
                    <div style={{ marginTop: 4 }}>
                      {d.status === "paid" ? <span className="badge-paid">Paid</span> : <span className="badge-pend">Pending</span>}
                    </div>
                  </div>
                ...
                <div style={{ display: "flex", gap: 7, marginTop: 10 }}>
                  <button className="btn-gold" style={{ padding: "6px 12px", fontSize: 11 }} onClick={() => mark(d.id, d.status === "paid" ? "pending" : "paid")}>
                    {d.status === "paid" ? "Mark Pending" : "Mark Paid"}
                  </button>
                  <button
                    className="btn-out"
                    style={{ padding: "6px 12px", fontSize: 11, color: "#EF4444", borderColor: "rgba(239,68,68,0.35)" }}
                    onClick={() => del(d.id)}
                  >
                    Delete
                  </button>
            ...
            businesses.map((b) => (
              <div key={b.id} className="card">
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ color: "#F0F6FF", fontWeight: 700, fontSize: 14 }}>{b.name}</div>
                    <div style={{ color: "rgba(155,181,224,0.5)", fontSize: 12 }}>{b.service}</div>
                    <div style={{ color: "rgba(155,181,224,0.4)", fontSize: 11 }}>{b.category}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div className="gold" style={{ fontWeight: 800, fontSize: 13 }}>{b.price || "--"}</div>
...
export default function App() {
      ...
      if (u) {
        try {
          ...
          if (snap.exists()) {
            ...
            await setDoc(doc(db, "users", u.uid), fallback, { merge: true });
            setProfile({ ...fallback, createdAt: null });
          }
        } catch (e) {
          console.error(e);
          setProfile({
  ...
  const navItems = [
    ...
    { key: "charity", icon: "🏛️", label: "Donations" },
    { key: "business", icon: "🛒", label: "Business" },
    { key: "profile", icon: "👤", label: "Profile" },
  ];

  if (ctxValue.isAdmin) {
    navItems.push({ key: "admin", icon: "⚙️", label: "Admin" });
  }
  ...
  return (
    <Ctx.Provider value={ctxValue}>
      ...
      <div className="app">
        ...
        {tab === "profile" && <Profile />}
        {tab === "admin" && ctxValue.isAdmin && <Admin />}

        <div className="bottom-nav">
          {navItems.map((item) => (
            <button
              key={item.key}
              className={`bottom-btn ${tab === item.key ? "active" : ""}`}
